		...
		for (BlockPos pos : targetBlocks) {
			...
			if (playerHasOneSeed(player, seedItem)) {
				...
				Block blockFromSeed = ForgeRegistries.BLOCKS
						.getValue(((BlockNamedItem) seedItem).getBlock().getRegistryName()); // get the block to place
				...
				((ServerPlayerEntity) player).awardStat(Stats.ITEM_USED.get(seedItem)); // increase vanilla
																						// item-use-counter
